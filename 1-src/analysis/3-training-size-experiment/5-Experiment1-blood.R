...
names(sampleList) <- nr
saveRDS(sampleList, file="./5-output/sampleListloop.Rds") # fix list to repeat
...
saveRDS(out.list, file="./5-output/sampleListloopAges.Rds")
...
for (j in 1:length(sampleList)){
  ...
  saveRDS(out.df, file="./5-output/results_training_size.Rds") # overwrite results at end of each loop as a means of saving temporary result
}

### Check output one experiment ##----

# # accuracy/precision/features/age distribution
# 
# results_training_size <- out.df
# 
# results_training_size %>%
#   ggplot(aes(x=train.sample.size,y=val.slope.regress,color=method)) +
#   geom_point() +
...
# 
# results_training_size %>%
#   ggplot(aes(x=train.sample.size,y=rep.ICC,color=method)) +
...
# mu <- mean(out.list[[1]]$age)
# ggplot(data=out.list[[1]], aes(x=age)) +
#   geom_density() +
#   geom_vline(xintercept=mu,linetype="dashed") +
#   xlab(paste0("age in training set with ",names(out.list[1])," samples")) +
#   theme_minimal()
# 
# mu <- mean(out.list[[2]]$age)
# ggplot(data=out.list[[2]], aes(x=age)) +
#   geom_density() +
#   geom_vline(xintercept=mu,linetype="dashed") +
#   xlab(paste0("age in training set with ",names(out.list[2])," samples")) +
#   theme_minimal()
# 
# 
